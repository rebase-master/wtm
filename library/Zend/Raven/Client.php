...
class Raven_Client
{
    ...
    public function buildJavaData($data)
    {

        $filename = explode(':', $data['message']);
        $regex = '#\((([^()]+|(?R))*)\)#';
        preg_match_all($regex, $data['message'], $matches);
        $k = 0;
        while (stristr($matches[1][$k], "java:") == false) {
            $k++;
        }
        $line = explode(':', $matches[1][$k]);
        $frame = array(
            "abs_path" => $filename[0],
            "filename" => $filename[0],
            "lineno" => $line[1],
        ...
        $data['extra'] = $data['sentry.interfaces.Http']['data'];
        $data['sentry.interfaces.Stacktrace']['frames'] = array(0 => $frame);
        $data['sentry.interfaces.Exception'] = array(0 => $exception);
        $data['culprit'] = $filename[0];
        $data['logger'] = "java";
    ...
    public function send($data)
    {
        ...
        if (!empty($data['message'])) {
            if (stristr($data['message'], "java:") == true)
                $data = $this->buildJavaData($data);
        }
