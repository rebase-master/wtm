...
/*
 ...
 A little modification is done in the make_thumb() where src_x, src_y, crop_height, crop_width were
 given a value if they were not set.
 If the size of the image was smaller than thumbnail size these values were not set.
...
class ImgBrowz0r
{
	...
	public function browse()
	{
		...
		foreach ($this->files as $k => $file)
		{
			if ($file[0] === 1)
			{
				...
				$info = pathinfo($file[1]);
				$file_name =  basename($file[1],'.'.$info['extension']);
				$fileExtract=explode(" ",$file_name);
				
				$caption=$fileExtract[0];
				
				if(isset($fileExtract[1]))
					$caption.=" ".$fileExtract[1];
				if(isset($fileExtract[2]))
					$caption.="...";
				...
				echo "\t\t", '<div class="img-thumbnail img-column-', $t_row_count, '"><a href="', $this->config['images_url'],
					 ...
					 $image_thumbnail, '\')" rel="photo[gallery]" title="', $file_name , '">', $file_name, '</a><span>',  $caption,
				     '</span></div>', "\n";
			}
			else
			{
				if ($this->config['dir_thumbs'])
				{
					...
					echo "\t\t", '<div class="img-directory-container">','<div class="img-directory img-column-', $t_row_count, '"><a href="',
	...
	public function breadcrumbs()
	{
		...
		return '<p class="img-breadcrumbs"><span>You are here <img src="../images/breadcrumb_arrow.png" /> </span><a href="'.str_ireplace('%PATH%',  '0/1', $this->config['main_url']).
		       '"> gallery </a>'.(isset($output) ? '<img src="../images/breadcrumb_arrow.png" /> '.implode(' <img src="../images/breadcrumb_arrow.png" /> ', $output) : null).'</p>';
	}
	...
	protected function make_thumb($image_dir, $image_name, $image_thumbnail)
	{
		    ...
		    $image_info['height'] < $this->config['max_thumb_height'])
		{
			...
			//$src_x = mt_rand(0, abs($crop_width - $thumb_width));
			//$src_y = mt_rand(0, abs($crop_height - $thumb_height));
		}
		else
		{
		...
		if(!isset($src_x) && !isset($src_y) && !isset($crop_height) && !isset($crop_width)){
			$src_x=$src_y=0;
			$crop_height=$image_info['height'];
			$crop_width=$image_info['width'];
		}
...
class ImgBrowz0rCache
{
	...
	public function __construct($cache_path = '/tmp', $cache_lifetime = 31536000)
	{
